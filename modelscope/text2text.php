...
class text2text{
	...
	public static $model="";
	public static $baseurl="https://api-inference.modelscope.cn";
	public static function init($cfg=[]){
		$arr=explode(",",$cfg["modelscope_api_key"]);
		shuffle($arr);
		$api_key=$arr[0];
        self::$api_key= $api_key;
		if(!empty($cfg["modelscope_text2text_model"])){
			self::$model=$cfg["modelscope_text2text_model"];
		}else{
