...
class img2text{
	...
	public static function init($cfg=[]){
		self::$api_key=$cfg["ali_api_key"];
		$arr=explode(",",$cfg["ali_api_key"]);
		shuffle($arr);
		$api_key=$arr[0];
        self::$api_key= $api_key;
		if(!empty($cfg["ali_img2text_model"])){
			self::$model=$cfg["ali_img2text_model"];
		}else{
	...
	public static function stream_chat($prompt,$messages=[],$callback=null){
		...
		$res=self::post_json_sse("https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions",json_encode($ops),$callback);
		return $res;
	}
